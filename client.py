        ...
        if response['status'] == 0:
            if 'result' in response:
                ...
                if response['result']['final']:
                    ...
                    if (self.previous_trans.__getitem__(self.previous_trans.__len__()-1).__len__() - trans.__len__()) < 20:
...
def getTranscription(audiofile, uri ="ws://localhost:8080/client/ws/speech", rate = 32000, content_type = ''):
    ...
    ws = MyClient(audiofile, uri + '?%s' % (urllib.urlencode([("content-type", content_type)])),
                  byterate=rate)
    ws.connect()
...
def main():
    ...
    parser.add_argument('-u', '--uri', default="ws://localhost:8080/client/ws/speech", dest="uri", help="Server websocket URI")
